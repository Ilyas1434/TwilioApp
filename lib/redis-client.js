...
/**
 * Resolve an env var by suffix. Vercel's Marketplace integrations prefix the
 * injected variables with the database name (e.g. `luna_KV_REST_API_URL`), so
 * we match both the exact name and any `<prefix>_<suffix>` variant. This keeps
 * the client working no matter what the storage integration is named.
 */
...
function createRedisClient() {
  // Check for Upstash REST API credentials (preferred — HTTP-based, ideal for serverless)
  const upstashUrl =
